...
func get_data():
	...
	var data = {}
	data["type"] = "gn_process"
	data["name"] = name
	data["offset"] = offset
	data["runtype"] = $ob_runtype.get_item_text($ob_runtype.selected)
	data["runlength"] = $le_runlength.text
	data["chk_break"] = $break/chk_break.pressed
	data["break_condition"] = $gn_sub_if_statement.get_data()
	if $le_runlength.text == "":
		data["runlength"] = "0"
	data["process"] = $le_name.text
	data["slot_count"] = get_child_count()-6
	return data
...
func set_data(pdata):
	...
	$break/chk_break.pressed = pdata["chk_break"]
	$gn_sub_if_statement.set_data(pdata["break_condition"])
